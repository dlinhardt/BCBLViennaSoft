function drawFixation(d, colIndex, frame, Center, MoveType, MoveRadius, Array, ArraySize, ShiftStim)
%
...
if exist('Center','var') && ~isempty(Center)
    ...
    d.fixX=(d.fixX*2)*Center(1);
    d.fixY=(d.fixY*2)*(1-Center(2));
...
if exist('ShiftStim','var') && max(abs(ShiftStim))~=0
    d.fixX=d.fixX+ShiftStim(1);
    d.fixY=d.fixY+ShiftStim(2);
end
...
switch(lower(d.fixType))
    ...
    case {'disk','left disk','right disk', 'upper left', 'lower left', 'upper right', 'lower right', 'left', 'right', 'upper', 'lower'}
        ...
        if nargin < 5
            ...
            Screen('gluDisk', d.windowPtr, d.fixColorRgb(colIndex,:), d.fixX,d.fixY, d.fixSizePixels); % center
            %Screen('gluDisk', d.windowPtr, d.fixColorRgb(colIndex,:),128+256, 256, d.fixSizePixels);  % top left
            %Screen('gluDisk', d.windowPtr, d.fixColorRgb(colIndex,:), 128+768, 256, d.fixSizePixels); % top right
            %Screen('gluDisk', d.windowPtr, d.fixColorRgb(colIndex,:), 128+256, 768, d.fixSizePixels); % bottom left
            %Screen('gluDisk', d.windowPtr, d.fixColorRgb(colIndex,:),128+768, 768, d.fixSizePixels); % bottom right
            %Randomized:
            %qRandIntervall=50;6
            %Screen('gluDisk', d.windowPtr, d.fixColorRgb(colIndex,:), d.fixX-RandIntervall+(2*RandIntervall).*rand,d.fixY-RandIntervall+(2*RandIntervall).*rand, d.fixSizePixels); % rand with -10,10
            
            %CircleArray=[-radius-1:Speed:radius-1,radius-1:-Speed:-radius-1];
            %equally spaced - point changes speed
            
        else
            
            if strcmp(MoveType,'Circle')
                
                FinishedPeriods=floor(frame/(ArraySize));
                
                %If you can divide frame/radius - you've reached the final image
                if mod(frame,(ArraySize))==0
                    frame=frame-(FinishedPeriods-1)*ArraySize;
                else
                    frame=frame-FinishedPeriods*ArraySize;
                end
                
                Circlex=Array(frame);
                
                %Show speed of the dot for equally spaced CircleArray
                %SpeedX=abs(Circlex/sqrt(radius^2-Circlex^2))
                
                if frame<=ArraySize/2
                    Screen('gluDisk', d.windowPtr, d.fixColorRgb(colIndex,:), d.fixX+Circlex,d.fixY+sqrt(MoveRadius^2-(Circlex)^2), d.fixSizePixels); % rand with -10,10
                else
                    Screen('gluDisk', d.windowPtr, d.fixColorRgb(colIndex,:), d.fixX+Circlex,d.fixY-sqrt(MoveRadius^2-(Circlex)^2), d.fixSizePixels); % rand with -10,10
                end
                ...
                if isempty(Array)
                    ...
                    xPosition=Array(1,frame);
                    yPosition=Array(2,frame);
                    Screen('gluDisk', d.windowPtr, d.fixColorRgb(colIndex,:), d.fixX+xPosition,d.fixY+yPosition, d.fixSizePixels);
                end
    ...
    case {'my thin cross'},
        if numel(d.fixCoords) > 1, colIndex2=colIndex; else, colIndex2 = 1; end;
        if exist('ShiftStim','var') && max(abs(ShiftStim))~=0
            d.fixCoords{colIndex2}=bsxfun(@plus, d.fixCoords{colIndex2}, ShiftStim);
        end
        Screen('DrawDots', d.windowPtr, d.fixCoords{colIndex2}, d.fixSizePixels(colIndex2), d.fixColorRgb(colIndex,:));
        %Print also a dot in the centre
        %Screen('gluDisk', d.windowPtr, d.fixColorRgb(colIndex,:), d.fixX,d.fixY, d.fixSizePixelsFixationDisk);
        Screen('gluDisk', d.windowPtr, d.fixColorRgbFixationDisk(colIndex,:), d.fixX,d.fixY, d.fixSizePixelsFixationDisk);
